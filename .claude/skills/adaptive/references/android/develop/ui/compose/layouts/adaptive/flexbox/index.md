...
`FlexBox` is usually used to display a small number of items _within_ an
...
`FlexBox` lays out its items in either horizontal or vertical _lines_ . This
direction of these lines establishes the _main axis_ . 90 degrees to the main
axis is the _cross axis_ . The length of the `FlexBox` along the main axis is
known as the _main size_ . The corresponding cross axis length is known as the
_cross size_ . These sizes and axes form the basis of `FlexBox`'s behavior.
...
4. **Align or resize items in the main axis** : For each line, distribute extra
   space _to_ or between items by [resizing](https://developer.android.com/develop/ui/compose/layouts/adaptive/flexbox/item-behavior#item-size) or
